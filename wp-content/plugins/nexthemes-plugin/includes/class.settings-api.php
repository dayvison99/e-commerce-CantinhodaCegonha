...
class WeDevs_Settings_API {
    ...
    function callback_validate_api($args){
        ...
        if(empty($value_useravata)) $value_useravata = NEXTHEMES_PLUGIN_URL . 'assets/images/default_avatar.jpg';
        $size = isset( $args['size'] ) && !is_null( $args['size'] ) ? $args['size'] : 'regular';
        $type = isset( $args['type'] ) ? $args['type'] : 'text';
        $action = isset( $args['action'] ) ? $args['action'] : '';


        $html = sprintf('<div class="nth-validate-api nth-validate-instagram" data-action="%1$s" data-ajax_action="%1$s_validate_token" data-section="%2$s">', esc_attr($action), $args['section']);
        $html .= sprintf( '<input type="%1$s" class="%2$s-text nth-validate-userid" name="%3$s[instagram_userid]" value="%4$s"/>', 'hidden', $size, $args['section'], $value_userid );
        $html .= sprintf( '<input type="%1$s" class="%2$s-text nth-validate-username" name="%3$s[instagram_username]" value="%4$s"/>', 'hidden', $size, $args['section'], $value_username );
        $html .= sprintf( '<input type="%1$s" class="%2$s-text nth-validate-useravata" name="%3$s[instagram_avata]" value="%4$s"/>', 'hidden', $size, $args['section'], $value_useravata );
        $html .= sprintf( '<input type="%1$s" class="%2$s-text nth-validate-usercount" name="%3$s[instagram_count]" value="%4$s"/>', 'hidden', $size, $args['section'], $value_usercount );
        $html .= sprintf( '<input type="%1$s" class="%2$s-text nth-validate-data" id="%3$s[%4$s]" name="%3$s[%4$s]" value="%5$s"/>', 'text', $size, $args['section'], $args['id'], $value );
        $html .= sprintf( '<input type="button" class="nth-validate-submit button" value="%s">', esc_attr__('Validate', 'nexthemes-plugin'));
    ...
    function show_navigation() {
        //$html = '<h2 class="nav-tab-wrapper nth-nav-subtab">';
        $html = '<ul class="nth-nav-subtab">';
        ...
        <script>
            jQuery(document).ready(function($) {
                ...
                if (activetab != '' && $(activetab + '-tab').length ) {
                    ...
                    $('.nth-nav-subtab a:first').addClass('nav-tab-active');
                }
                $('.nth-nav-subtab a').click(function(evt) {
                    $('.nth-nav-subtab a').removeClass('nav-tab-active');
